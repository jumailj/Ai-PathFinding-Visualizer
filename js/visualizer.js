...
function generate_grid() {
    ...
    grid = new Array(grid_size_x).fill(0).map(() => new Array(grid_size_y).fill(0)); // assign array to grid.
    /*
        [1] [2]
        [3] [4]
        [5] [6]
...
function clear_grid() {
    ...
    if(!grid_clean) { // default value is 'true', if grid is generation value must be 'false';
        ...
        for (let i = 0; i < grid.length; i++ ) 
        {
            for (let j = 0; j < grid[0].length; j++ ) 
            {
                    ...
                    {
                        remove_wall(i,j);
                        // if the algroing runs, the id contains "cell_algo" "cell_path" must be removed. so gird must be clear
